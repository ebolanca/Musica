...
// Si se ejecuta en OMEN, asegurar que PM2 reinicie el proceso con el nuevo código y persista
if (os.hostname() !== 'PC-MSI') {
    const { execSync } = require('child_process');
    try {
        console.log('🚀 [OMEN] Reiniciando y guardando servicio musica en PM2...');
        execSync('npx pm2 restart musica || npx pm2 start server.js --name musica', { cwd: path.join(__dirname, '..'), stdio: 'inherit' });
        execSync('npx pm2 save', { stdio: 'inherit' });
        console.log('✅ [OMEN] PM2 actualizado y persistido.');
    } catch(e) {
